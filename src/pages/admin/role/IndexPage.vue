  ...
  <q-page class="q-pa-sm">
    ...
    >
      ...
      <template v-slot:top-left>
        <div class="text-h5 q-pr-lg">Role</div>
        <q-btn
          color="primary"
          icon="add"
          ...
          dense
        >
          <q-tooltip>Tambah Role </q-tooltip>
        </q-btn>
      </template>
...
const navigateToAddItem = () => {
  router.push("/dashboard/role/add"); // Replace with your actual URL
};

//Edit Item form
const navigateToEditItem = (id) => {
  // console.log(slug);
  router.push(`role/edit/${id}`); // Replace with your actual URL
};
...
const deleteItemDialog = (row) => {
  $q.dialog({
    ...
    try {
      await useRoleStore().delete(row.uuid);
