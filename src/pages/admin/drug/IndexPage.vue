  ...
  <q-page class="q-pa-sm">
    ...
    >
      ...
      <template v-slot:top-left>
        <div class="text-h5 q-pr-lg">Drug</div>
        <q-btn
          color="primary"
          icon="add"
          ...
          dense
        >
          <q-tooltip>Tambah Drug </q-tooltip>
        </q-btn>
      </template>
...
const getItem = async () => {
  try {
    const res = await useDrugStore().all();
...
const navigateToAddItem = () => {
  router.push("/dashboard/drug/add"); // Replace with your actual URL
};

//Edit Item form
const navigateToEditItem = (id) => {
  // console.log(slug);
  router.push(`drug/edit/${id}`); // Replace with your actual URL
};
...
const deleteItemDialog = (row) => {
  $q.dialog({
    ...
    try {
      await useDrugStore().delete(row.uuid);
