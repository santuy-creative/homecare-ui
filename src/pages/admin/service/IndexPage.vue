  ...
  <q-page class="q-pa-sm">
    ...
    >
      ...
      <template v-slot:top-left>
        <div class="text-h5 q-pr-lg">Service</div>
        <q-btn
          color="primary"
          icon="add"
          ...
          dense
        >
          <q-tooltip>Tambah Service </q-tooltip>
        </q-btn>
      </template>
...
const navigateToAddItem = () => {
  router.push("/dashboard/service/add"); // Replace with your actual URL
};

//Edit Item form
const navigateToEditItem = (id) => {
  // console.log(slug);
  router.push(`service/edit/${id}`); // Replace with your actual URL
};
...
const deleteItemDialog = (row) => {
  $q.dialog({
    ...
    try {
      await useServiceStore().delete(row.uuid);
