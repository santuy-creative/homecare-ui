  ...
  <q-page class="q-pa-sm">
    ...
    >
      ...
      <template v-slot:top-left>
        <div class="text-h5 q-pr-lg">Payment</div>
        <q-btn
          color="primary"
          icon="add"
          ...
          dense
        >
          <q-tooltip>Tambah Payment </q-tooltip>
        </q-btn>
      </template>
...
const getItem = async () => {
  try {
    const res = await usePaymentStore().all();
...
const navigateToAddItem = () => {
  router.push("/dashboard/payment/add"); // Replace with your actual URL
};

//Edit Item form
const navigateToEditItem = (id) => {
  // console.log(slug);
  router.push(`payment/edit/${id}`); // Replace with your actual URL
};
...
const deleteItemDialog = (row) => {
  $q.dialog({
    ...
    try {
      await usePaymentStore().delete(row.uuid);
